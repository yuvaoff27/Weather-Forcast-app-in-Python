...
def initialize_db():
    ...
    # Drop the table if it exists (to prevent mismatches)
...
def store_search(city_name):
    conn = sqlite3.connect("weather_searches.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO search_history (city_name) VALUES (?)", (city_name,))
    conn.commit()
    conn.close()

# Call initialize_db AFTER defining it
initialize_db()

# GUI and Weather Logic (Your existing code follows here...)
root = Tk()
...
def getWeather():
    ...
    if not city:  # Prevent empty searches
    ...
    # Store city name in the database
    store_search(city)

    # Rest of the getWeather() function remains unchanged...


    #weather
    api="https://api.openweathermap.org/data/2.5/onecall?lat="+str(location.latitude)+"&lon="+str(location.longitude)+"&units=metric&exclude=hourly&appid=fdb198348c918bc192142bd2eff56da4"
    ...
    #tomorrow secondbox
    seconddateimage = json_data['daily'][1]['weather'][0]['icon']

    img= (Image.open(f"icon/{seconddateimage}@2x.png"))
    ...
    ##third box
    thirddateimage = json_data['daily'][2]['weather'][0]['icon']

    img= (Image.open(f"icon/{thirddateimage}@2x.png"))
    resized_image= img.resize((50,50))
    ...
    ##fouth box
    fourthdateimage = json_data['daily'][3]['weather'][0]['icon']

    img= (Image.open(f"icon/{fourthdateimage}@2x.png"))
    resized_image= img.resize((50,50))
    ...
    ##fifth box
    fifthdateimage = json_data['daily'][4]['weather'][0]['icon']

    img= (Image.open(f"icon/{fifthdateimage}@2x.png"))
    resized_image= img.resize((50,50))
    ...
    ##sixth box
    sixthdateimage = json_data['daily'][5]['weather'][0]['icon']

    img= (Image.open(f"icon/{sixthdateimage}@2x.png"))
    resized_image= img.resize((50,50))
    ...
    ##seventh box
    seventhdateimage = json_data['daily'][6]['weather'][0]['icon']

    img= (Image.open(f"icon/{seventhdateimage}@2x.png"))
    resized_image= img.resize((50,50))
